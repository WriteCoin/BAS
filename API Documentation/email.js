 ...
 * Чтобы извлекать заголовки такие как 'SUBJECT' или 'FROM', примените регулярное выражение к переменной [[MAIL_BODY]]. Например, чтобы извлечь заголовок 'FROM', используйте регулярное выражение From\:\s+(.*)
 * Для правильной работы необходимо сначала выполнить действие 'Настроить'.
 * @param {number} timeout Максимальное время выполнения задания
 ...
 * 
 * link1, link2, link3 - Переменные со ссылками. Ссылки будут автоматически извлечены из текста письма и помещены в переменные [[LINK1]], [[LINK2]] и [[LINK3]]. Вы можете назвать переменные по-другому или добавить новые, чтобы извлечь больше ссылок.
 */
