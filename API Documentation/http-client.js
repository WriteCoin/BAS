 ...
 * Вы можете сравнить статус действием 'If', например [[SAVED_STATUS]] == 200. Это выражение проверяет, был ли запрос успешным.
 ...
 * Вот некоторые примеры кода для работы с cookies. Все они могут быть помещены в действие 'Выполнить код'. Прежде чем использовать их, вы должны получить объект cookies из строки, например так, [[COOKIES]] = JSON.parse([[SAVED_COOKIES]])["cookies"]
 * Найти значение cookie по названию: [[COOKIE]] = [[COOKIES]].filter(function(el){return el["name"] == "ЗАМЕНИТЬ НА НАЗВАНИЕ COOKIE"})[0]
 * Проверить, существует ли cookie: [[COOKIE_EXISTS]] = [[COOKIES]].filter(function(el){return el["name"] == "ЗАМЕНИТЬ НА НАЗВАНИЕ COOKIE"}).length > 0
 * Получить значение cookie [[COOKIE_VALUE]] = [[COOKIE]]["value"]
 * Вы также можете изменить значение cookie [[COOKIE]]["value"] = "My value". Но изменение этого объекта не повлечет за собой изменение cookies внутри http-клиента, для этого вам нужно преобразовать cookies в строку(с помощью JSON.stringify([[COOKIES]])) и использовать его с действием 'HTTP-Клиент Загрузить cookies'.
 * Переменная [[COOKIES]] будет содержать список cookies, и так как это список, то его также можно обработать с помощью действий из модуля 'Список'.
 ...
 * Проверить, существует ли cookie: [[COOKIE_EXISTS]] = [[COOKIES]].filter(function(el){return el["name"] == "ЗАМЕНИТЬ НА НАЗВАНИЕ COOKIE"}).length > 0
 * Получить значение cookie [[COOKIE_VALUE]] = [[COOKIE]]["value"]
 * Вы также можете изменить значение cookie [[COOKIE]]["value"] = "My value".
 * После того, как вы закончите редактирования списка [[COOKIES]], вам нужно преобразовать его в строку с помощью кода JSON.stringify([[COOKIES]]) ( скрин https://wiki.bablosoft.com/lib/exe/fetch.php?cache=&media=cookiesrestore2.png).
 * Переменная [[COOKIES]] будет содержать список cookies, и так как это список, то его также можно обработать с помощью действий из модуля 'Список'.
